...
int ww_window_create(ww_window_t *window, char* title, int width, int height) {
...
int ww_window_destroy(ww_window_t window) {
	if(!window) {
...
int ww_window_update_events(ww_window_t window) {
	if(!window) {
...
int ww_window_received_quit_event(ww_window_t window) {
	if(!window) {
		fprintf( stderr, "Cannot update non-existent window\n" );
		return -1;
	}
...
int ww_window_update_buffer(ww_window_t window, ww_pixel_buffer_t pixels) {
	if(!window) {
		return -1;
	}
	ww_window_s *window_p = (ww_window_s*) window;
...
int main( void ) {
	...
	ww_pixel_buffer_t buffer = calloc(640 * 480, sizeof(int));
	...
	if(ww_window_create(&window, "Testing", 640, 480)) {
		printf("Failure\n");
		return 1;
	}
	
	while(!ww_window_received_quit_event(window)) {
		ww_window_update_events(window);
		ww_window_update_buffer(window, buffer);
	}
